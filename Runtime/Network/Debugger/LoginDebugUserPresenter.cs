...
namespace MokomoGamesLib.Runtime.Network.Debugger
{
    public class LoginDebugUserPresenter : MonoBehaviour
    {
        ...
        public void Initialize(LoginUserIdList loginUserIdList)
        {
            ...
            addUserButton.onClick.AddListener(() => { onClickedAddUserButton?.Invoke(addUserIdInputField.text); });

            closeButton.onClick.RemoveAllListeners();
            closeButton.onClick.AddListener(() => { onClickedClose?.Invoke(); });
        ...
        {
            foreach (Transform child in scrollRect.content.transform) Destroy(child.gameObject);
            ...
            _loginDebugUserViews = loginUserIdList.idList
                ...
                .ToList();
            _loginDebugUserViews.ForEach(x => { x.OnClickedCell += id => { onClickedLoginUserButton?.Invoke(id); }; });
        }
    }
}